...
INSERT INTO vocab.concept_ns_staging (concept_id,
                                      ...
                                      synonym,
                                      predicate_id)
SELECT row_number() OVER (ORDER BY source_concept_code) + (SELECT COALESCE(max(concept_id),2081500000) FROM vocab.concept WHERE concept_id < 2082000000 AND concept_id > 2081500000) AS concept_id,
...
SELECT row_number() OVER (ORDER BY source_concept_code) + (SELECT count(*) FROM vocab.mapping_metadata) AS mapping_concept_id,
       ...
       'https://docs.google.com/spreadsheets/d/13LvLymaCRa3vO6jk_I48qjEr6c_0BKuaTvhLdbuAEtY/edit?usp=sharing' AS mapping_provider,
       1,
       'Polina Talapova',
       COALESCE(rid.id, 0),
       reviewer_name,
